...
function onClickEv(child) {
  ...
  var wihe = 'width='+screen.availWidth+',height='+screen.availHeight;
    ...
    setTimeout(function(){
     window.open("http://cneas.test/famy/trackLocation/" + child.id, child.id, "resizable,scrollbars,status,screenX=1,screenY=1,left=1,top=1,"  + wihe);
},
500);
     //if (isHasAlert()){
     try{
...
function displayModal(content) {
    var sound = new Audio('/sound/notification.mp3');
    sound.play();
    document.getElementById('mt').textContent = "FAMY SENYAS";
